...
// -------------------------------------------------------------------------------- 
// FACTORY: It basically interacts with the rails server and processes the json response.
// -------------------------------------------------------------------------------- 

    app.factory('Items', ['$resource',function($resource){

      return $resource('http://quickandclean.org/categories.json', {},{
    ...
    app.factory('Item', ['$resource', function($resource){
      ...
      return $resource('http://quickandclean.org/categories/:id.json', {}, {
        show: { method: 'GET' },
...
app.factory("Item", function($http) {
  ...
  // var url = "http://systemaker.com/angulardemo/data/items.json?alt=json-in-script&callback=JSON_CALLBACK";
   ...
   var url = "http://quickandclean.org/categories.json?alt=json-in-script&callback=JSON_CALLBACK";
  ...
  $http.get(url).success(function(response){
    //items = JSON.stringify(response);  //ajax request to fetch data into items
    items = response; 
    console.log(items);
  }).error(function (data) {
...
// MAIN CONTROLLER
//-------------------------------------------------------------------------------------
app.controller("MainCtrl", function($scope, $location, tmhDynamicLocale, $translate, Items, Item, flash, $rootScope){

  // MAIN DATA BINDING
  $scope.navbarCollapsed = true;
  $scope.title = 'AngularJS Client for RESTful APIs web service';  
...
//-------------------------------------------------------------------------------------
app.controller("IndexCtrl", function($scope, $location, Items, Item) {
...
//-------------------------------------------------------------------------------------
  ...
  // NEW ITEM CTRL
  //-------------------------------------------------------------------------------------
  app.controller("NewCtrl", function($scope, $routeParams, Items, $location) {
    //$scope.item = Item.show($routeParams.id);
    $scope.new = {id: Items.index().length + 1};
    $scope.submitItem = function() {
        Items.create({category: $scope.new}, function(){
          $location.path("/items");
        }, function(error){
          console.log(error)
        });
  ...
  app.controller("EditCtrl", function($scope, $routeParams, Item, $location) {
    //$scope.item = Item.show($routeParams.id);
    $scope.new = Item.show({id:$routeParams.id});
    $scope.submitItem = function() {
        Item.update($scope.new,function(){
  ...
  // DELETE ITEM CTRL
  //-------------------------------------------------------------------------------------
  app.controller("DeleteCtrl", function($scope, $routeParams, Item, Items, $location) {
    //$scope.item = Item.show($routeParams.id);
          Item.destroy({ id: $routeParams.id }, function(){
            $scope.items = Items.index();   // After delete category get categories collection.
            $location.path("/items");
          });
  });
